...
vault-0 -- vault write auth/kubernetes/config \
  token_reviewer_jwt="$(kubectl get secret -n namespace_name $(kubectl get serviceaccount vault-auth -n default -o jsonpath='{.secrets[0].name}') -o 
  jsonpath='{.data.token}' | base64 --decode)" \
  kubernetes_host="$(kubectl config view --raw -o jsonpath='{.clusters[0].cluster.server}')" \
  kubernetes_ca_cert="$(kubectl get secret -n namespace_name $(kubectl get serviceaccount vault-auth -n default -o jsonpath='{.secrets[0].name}') -o 
